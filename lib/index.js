...
var EZFlux = function (_EventEmitter) {
  ...
  function EZFlux() {
    ...
    _this.history = {};
    _this.cfg = { throttleUpdates: false, log: { events: false, histor: false, trace: false } };
  ...
  _createClass(EZFlux, [{
    ...
    value: function emitOrBuffer(eventName, id) {
      ...
      this.emissionTimeout = window.requestAnimationFrame(function () {
        ...
        for (var i = names.length; i--;) {
          var _ids = _this4.eventBuffer[names[i]];

          _this4.emit(names[i], _ids);
    ...
    value: function emit() {
      ...
      this.history[time] = { time: time, name: name, ids: id, state: this.state };
      ...
      (_console = console)[this.cfg.log.trace ? 'trace' : 'log'].apply(_console, logArgs); // eslint-disable-line no-console
    }
