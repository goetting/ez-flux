  ...
  _createClass(Flux, [{
    ...
    value: function createStores(storeConfigs) {
      ...
      Object.keys(storeConfigs).forEach(function (name) {
        return _this2.createStore(name, storeConfigs[name]);
      });
    }
  }, {
    key: 'createStore',
    value: function createStore(name, storeConfigParam) {
      ...
      if (storeConfig.isValid) {
        ...
        this.stateGetters[name] = {
          get: function get() {
            return Object.assign({}, _this3.state[name]);
          },
          getTypes: function getTypes() {
            return Object.assign({}, storeConfig.stateTypes);
          }
        };
    ...
    value: function integrateReactions(_ref2) {
      var _this4 = this;
      ...
      Object.keys(reactions).forEach(function (reactionName) {
        var evName = _this4.createAction(name, reactionName);

        _this4.bus.on(evName, function (data) {
          var newState = _this4.dispatch(reactions[reactionName], data, name);
          var newStateIsValid = Object.keys(newState).findIndex(function (key) {
            ...
            if (isInValid) {
              _this4.logger.error(name + '.reactions.' + reactionName + ':' + ('stateKey ' + key + ' had value ' + newState[key] + ' but should be type ' + stateTypes[key]));
            }
            return isInValid;
          }) > -1;

          if (newStateIsValid) {
            Object.assign(_this4.state[name], state);
            _this4.queueUpdate(name);
          }
        });
      });
    }
    ...
    value: function createAction(store, actionName) {
      ...
      this.actions[store][actionName] = function (data) {
        if (_this5.config.bufferActions && _this5.isDispatching) {
          _this5.dispatchBuffer.push({ evName: evName, data: data });
        } else {
          _this5.bus.emit(evName, data || {});
        }
      };
    ...
    */
    ...
    value: function dispatch(reaction, data, storeName) {
      ...
      var newState = reaction(data, this.state[storeName]);
      this.isDispatching = false;
      this.flushDispatchBuffer();
